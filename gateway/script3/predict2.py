...
import pandas as pd
import numpy as np
# import joblib  <-- Tạm thời comment dòng này nếu không muốn load thư viện nặng
import time
import sys
import os
import csv
...
THRESHOLD_RATE = 5000   # Rate > 1000 -> Suspected DoS
...
def call_api_for_alert(label_text, flow_count, method="Unknown"):
    print("---------------------------------")
...
def run_predictor(model):
    try:
        ...
        # ---------------------------------------------------------
        # STEP 4: [MODIFIED] SKIP ML - ASSIGN DEFAULT
        # ---------------------------------------------------------
        
        mask_ml = df_full['Predicted_Label'].isnull()
        
        if mask_ml.any():
            # Thay vì gọi model.predict, ta gán thẳng là Benign
            # print(f"[Test Mode] Skipping ML for {mask_ml.sum()} flows -> Assigning 'Benign'")
            
            df_full.loc[mask_ml, 'Predicted_Label'] = "Benign"
            df_full.loc[mask_ml, 'Detection_Method'] = "ML-Disabled (Test Mode)"
...
if __name__ == "__main__":
    print(f"[*] [Module 3] Starting Detection System...")
    
    # [MODIFIED] Không load Model nữa
    model = None
